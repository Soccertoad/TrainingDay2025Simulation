...
import frc.robot.util.BearUtil;
...
public class SwerveSetpointGenerator {
  ...
  public SwerveSetpointGenerator(
      final SwerveDriveKinematics kinematics, Translation2d[] modulelocations) {
  ...
  private double findRoot(
      ...
      double f_1,
      int iterations_left) {
    if (iterations_left < 0 || BearUtil.epsilonEquals(f_0, f_1)) {
      return 1.0;
    }
  ...
  public SwerveSetpoint generateSetpoint(
    ...
    if (BearUtil.epsilonEquals(BearUtil.chassisSpeedsToTwist2d(desiredState), new Twist2d())) {
      need_to_steer = false;
    ...
    for (int i = 0; i < modules.length; ++i) {
      ...
      if (all_modules_should_flip) {
        double required_rotation_rad =
            Math.abs(prev_heading[i].times(-1).rotateBy(desired_heading[i]).getRadians());
    ...
    if (all_modules_should_flip
        && BearUtil.epsilonEquals(
            BearUtil.chassisSpeedsToTwist2d(prevSetpoint.chassisSpeeds), new Twist2d())
        && !BearUtil.epsilonEquals(BearUtil.chassisSpeedsToTwist2d(desiredState), new Twist2d())) {
    ...
    for (int i = 0; i < modules.length; ++i) {
      ...
      if (BearUtil.epsilonEquals(prevSetpoint.moduleStates[i].speedMetersPerSecond, 0.0)) {
        ...
        if (BearUtil.epsilonEquals(desiredModuleState[i].speedMetersPerSecond, 0.0)) {
        ...
        var necessaryRotation =
            prevSetpoint.moduleStates[i].angle.times(-1).rotateBy(desiredModuleState[i].angle);
    ...
    for (int i = 0; i < modules.length; ++i) {
      ...
      final var deltaRotation =
          prevSetpoint.moduleStates[i].angle.times(-1).rotateBy(retStates[i].angle);
